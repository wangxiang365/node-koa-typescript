...
let filterIdCard = (idcard) => {
  ...
  switch (idcard.length) {
    ...
    case 8:
      return IsHKID(idcard)
    case 10:
      return isTWID(idcard)
    default:
...
let getUserInfo = () => {
  return new Promise((resolve, reject) => {
    Vue.prototype.$http.post(`${Vue.prototype.api.personalInfo}`, {
      id: JSON.parse(localStorage.getItem('LZWLUSERINFO')) ? JSON.parse(localStorage.getItem('LZWLUSERINFO')).userId : ''
    }).then((res) => {
      if (res.data.status !== Vue.prototype.api.ERR_OK || !res) return false
