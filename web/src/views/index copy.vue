<template>
  <div class="home-wrap">
    ...
    <!-- title -->
    <div class="tit">
      <img src="../assets/images/ind-tit.png">
    </div>
    <!-- list -->
    <div class="list">
      <ul>
        <li v-for="(item, index) in mealList" :key="index">
          ...
          <div class="price">
            ...
            <div @click="item.whetherNumberLimit === 2 && item.userBuyCount >= item.numberLimit ? '' : $router.push('/goodsDetail')" :class="{disabled: item.whetherNumberLimit === 2 && item.userBuyCount >= item.numberLimit}">{{item.whetherNumberLimit === 2 && item.userBuyCount >= item.numberLimit ? '已订购':'立即订购'}}</div>
          </div>
        </li>
      </ul>
    </div>
