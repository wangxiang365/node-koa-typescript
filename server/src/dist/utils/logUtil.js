...
const logPath = config_1.default.logs.logPath || "./appLogs/"; // 通过环境变量配置日志路径，默认在运行目录下新建app_log文件夹
// 行号
function lineno() {
    ...
    */
    _initCommon(name) {
        this._commonLogger = createLogger({
            format: format.combine(format.errors({ stack: false }), format.simple(), format.colorize()),
            transports: [
                new transports.File({
        ...
        if (process.env.NODE_ENV !== "formal") {
            this._commonLogger.add(new transports.Console({
                format: format.combine(format.errors({ stack: false }), format.simple()),
            }));
        }
    }
    /**
